...
**Procedure**: [casestudy_flooder.aal](procedure.aal)

**Topology** : [casestudy_flooder.tcl](topology.tcl)

**Archived logs**:  [casestudy_flooder.tar.gz](cs_exparchive_flooder.tar.gz)
...
A plot of the traffic on the link containing the flooder and the victim server can be generated by the [MAGI Graph Creation Tool](magi-tools/#magi_graph.py).
...
![Graph](traffic_plot.png)

**Procedure**   :   [flooder_monitor.aal](flooder_monitor.aal)

**Archived logs**:  [casestudy_flooder_monitor.tar.gz](cs_exparchive_flooder_monitor.tar.gz)

**Graph Config**:   [graph.conf](graph.conf)